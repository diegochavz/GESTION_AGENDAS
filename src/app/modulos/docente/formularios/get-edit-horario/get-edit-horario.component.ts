...
@Component({
  selector: 'app-get-edit-horario',
  templateUrl: './get-edit-horario.component.html',
  styleUrls: ['./get-edit-horario.component.scss']
})
export class GetEditHorarioComponent implements OnInit {
  ...
  constructor(private estudianteService: EstudianteServiceImpl,
              public dialogRef: MatDialogRef<GetEditHorarioComponent>,
    ...
    this.listHorariosEdit = data.horariosEdit;
  }
  ...
  compareNewHorarioList(horario: Horario): boolean {
    ...
    let listEditSinHorario = this.formularioHelps.sacarHorarioDeList(this.listHorariosEdit, this.horario)
    let concat_uno = this.listHorariosOcupados.concat(this.listHorarosAgregados);
    let concat_dos = concat_uno.concat(listEditSinHorario);
    for (let i = 0; i < listHorariosNew.length; i++) {
      for (let j = 0; j < concat_dos.length; j++) {
        if (this.formularioHelps.validarCruceFecha(listHorariosNew[i], this.formularioHelps.desglosarHorarios(concat_dos[j]))) {
          return true;
        }
      }
    }
  ...
  updateHorario() {
    this.horario.fecha_inicial = moment(this.formHorario.get('fecha_inicial').value).format("YYYY-MM-DD")
    this.horario.fecha_final = moment(this.formHorario.get('fecha_final').value).format("YYYY-MM-DD")
    this.horario.inicio_horario = this.formHorario.get('inicio_horario').value
    this.horario.fin_horario = this.formHorario.get('fin_horario').value
    this.horario.se_repite = this.formHorario.get('repeticion').value
    if (this.horario.se_repite) {
      let aux = [];
      if (this.formHorario.get('domingo').value) {
        aux.push(0)
      }
